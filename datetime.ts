export class TimeSpan {
    ...
    public Duration(): TimeSpan {
        if (this.Millis === TimeSpan.MinValue.Millis) {
    ...
    public Multiply(factor: number): TimeSpan {
        ...
        const num = Math.round(this.Millis * factor);
    ...
    public Divide(divisor: number): TimeSpan {
        ...
        const num = Math.round(this.Millis / divisor);
        if (Number.isNaN(num)) {
            throw new Error(`Overflow_TimeSpanTooLong`);
        }
        return TimeSpan.FromMillis(num);
...
export class DateTime {
    ...
    public get TimeOfDay(): TimeSpan {
        return new TimeSpan(this.Millis - this.Date.Millis);
    }
    ...
    public Add(value: TimeSpan): DateTime {
        return this.AddMillis(value.Millis);
    }
